...
@section('content')
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        @if (session('success'))
            <div class="px-6 py-4 text-green-700 bg-green-100 border border-green-200 rounded mb-4">{{ session('success') }}
            </div>
        @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                @if (can('عرض مستخدم'))
                                    <a href="{{ route('admin.roles.show', $role) }}"
                                        class="text-blue-600 hover:text-blue-900 p-1 rounded">عرض</a>
                                @endif
                                @if (can('تعديل مستخدم'))
                                    <a href="{{ route('admin.roles.edit', $role) }}"
                                        class="text-indigo-600 hover:text-indigo-900 p-1 rounded">تعديل</a>
                                @endif
                                @if (can('حذف مستخدم'))
                                    <form action="{{ route('admin.roles.destroy', $role) }}" method="POST"
                                        class="inline-block" onsubmit="return confirm('هل أنت متأكد من حذف هذا الدور؟')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-red-600 hover:text-red-900 p-1 rounded">حذف</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
