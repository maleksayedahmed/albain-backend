        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('الصورة') }}</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($partner->getFirstMediaUrl('image'))
                                    <img src="{{ $partner->getFirstMediaUrl('image') }}" alt="{{ $partner->name }}" class="w-12 h-12 object-cover object-center rounded-lg mx-auto" style="aspect-ratio:1/1;max-width:48px;max-height:48px;">
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2 space-x-reverse">
                                    ...
                                    <form method="POST" action="{{ route('admin.partners.destroy', $partner) }}"
                                        class="inline-block" onsubmit="return confirm('هل أنت متأكد من حذف هذا الشريك؟')">
                                        @csrf
                                        @method('DELETE')
